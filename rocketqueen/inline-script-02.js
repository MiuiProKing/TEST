  ...
  const STORAGE_KEY = "lumorax_rocketqueen_live_v1";
  ...
  function acceptAviatorMessage(data){
    ...
    if(data.type === "bridge-ready" || data.type === "heartbeat"){
      ...
      setApiState("ROCKET QUEEN " + aviatorRoundsReceived + " LIVE", "ok");
      return true;
    }
    ...
    if(data.type === "history"){
      ...
      if(added){
        ...
        setStatus("Rocket Queen synchronized: " + added + " new rounds received.", "ok");
  ...
  if(aviatorBtn) aviatorBtn.addEventListener("click", () => {
    ...
    if(nativeControl){
      nativeControl.postMessage({ action:"open-rocketqueen", hasState:Boolean(localStorage.getItem(STORAGE_KEY)) });
      setStatus("Открываю Rocket Queen внутри приложения. После входа мост передаст коэффициенты.", "ok");
      writeAnalysis("Войдите в 1win в открывшемся окне и запустите Rocket Queen. Ставки приложение не нажимает.", "ok", analyze());
      return;
    }
    const gameWindow = window.open("https://one-vv1220.com/casino/play/v_1wingames:rocketqueen?p=yshe", "lumorax-aviator");
    if(gameWindow){
      setStatus("Rocket Queen открыта. Мост передаст отображённые завершённые коэффициенты.", "ok");
      writeAnalysis("Жду коэффициенты Rocket Queen. Сигналы появятся после получения истории.", "ok", analyze());
    }else{
      setStatus("Не удалось открыть Rocket Queen.", "ko");
    }
  });
  ...
  renderHistory();
  setButtons(last5.length >= 5);
  setApiState(restoredState ? "ROCKET QUEEN RESTORED" : "WAIT ROCKET QUEEN", restoredState ? "ok" : "");
  if(last5.length >= 5){
  ...
  if(!aviatorBridgeReady) setApiState(restoredState ? "ROCKET QUEEN RESTORED" : "WAIT BRIDGE", restoredState ? "ok" : "");
