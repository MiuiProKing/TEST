...
static void update_button_title(void) {
    if (!g_button) return;
    const char *labels[] = {"🎛 МОДЫ +8","🎛 KS4","🎛 KS5+V4A","🎛 KS6","🎛 STEREO","🎛 DUAL","🎛 WHYRED","🎛 aptX EQ","🎛 INTERFACE"};
...
static void show_profile_menu(id self, SEL _cmd) {
    (void)self; (void)_cmd;
    id alert = M3(id,C("UIAlertController"),"alertControllerWithTitle:message:preferredStyle:",id,nsstr("🎛 Расширенные пресеты"),id,nsstr("Выбери дополнительный аудио-профиль. Родные Обычный / Бас / Глубокий бас остаются в левой части строки Пресет."),NSInteger,0);
    const char *names[] = {
...
static void install_button(id self, SEL _cmd) {
    ...
    CGRect f = {{bounds.size.width - 158.0, 156.0},{128.0,42.0}};
    ...
    M1(void,layer,"setCornerRadius:",CGFloat,14.0);
